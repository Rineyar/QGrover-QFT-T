...
int search_amp_by_idx(const State *state, int idx); //Поиск индекса по её индексу

int set_amp_by_idx(State *state, complex double amp, int idx); //Установка нового значения амплитуды

complex double read_amp_by_idx(const State *state, int idx); //Получение числового значения амплитуды